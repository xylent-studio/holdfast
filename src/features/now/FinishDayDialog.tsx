    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Finish day">
      <div className="dialog-stack">
        <div>
          ...
          <h2>Finish day</h2>
          <p>Leave tomorrow a cleaner start than you got today.</p>
        </div>
        <label className="field-stack">
          <span>What landed</span>
          <textarea onChange={(event) => setCloseWin(event.target.value)} rows={3} value={closeWin} />
        </label>
        <label className="field-stack">
          <span>What still matters</span>
          <textarea onChange={(event) => setCloseCarry(event.target.value)} rows={4} value={closeCarry} />
        </label>
        <label className="field-stack">
          <span>Tomorrow seed</span>
          <textarea onChange={(event) => setCloseSeed(event.target.value)} rows={3} value={closeSeed} />
        </label>
        <label className="field-stack">
          <span>Note</span>
          <textarea onChange={(event) => setCloseNote(event.target.value)} rows={3} value={closeNote} />
        </label>
