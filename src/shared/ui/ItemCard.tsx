...
function countAttachments(attachments: AttachmentRecord[]): string | null {
  ...
  const photos = attachments.filter((attachment) => attachment.kind === 'image').length;
...
export function ItemCard({
  ...
  const preview = previewText(item.body, item.kind === 'note' ? 220 : 120);

  return (
    <article className={`item-card ${item.kind === 'note' ? 'note' : 'task'} ${focus ? 'focus' : ''}`}>
      <div className="item-row">
        ...
        <div className="item-copy">
          ...
          {attachmentSummary ? <div className="attachment-summary">{attachmentSummary}</div> : null}
      ...
      <div className="item-actions">
        {primaryActionLabel && onPrimaryAction ? (
          <button className="button accent" onClick={onPrimaryAction} type="button">
            {primaryActionLabel}
          </button>
        ) : null}
